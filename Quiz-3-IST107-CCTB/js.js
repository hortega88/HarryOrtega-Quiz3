...
function initialize() {
  ...
  //setInterval(showOptions, 500);error popup
  // debugger;
}
...
function endGame(who) {
  if (who == player) {
    announceWinner(`Congratulations, ${userName} won!`);
...
function submitName() {
  ...
  if (userName) {
    // Update the text in the modal with the user's name
    document.getElementById("userName").textContent = userName;
  } else {
    // If no name is entered, display a default message or alert
